...
output_stds = xr.open_dataset(config["stds_output"])#stds_output": "/nesi/project/niwa00018/ML_downscaling_CCAM/multi-variate-gan/inputs/normalization/target_spatial_norm_all_gcm_std.nc", "means_output": "/nesi/project/niwa00018/ML_downscaling_CCAM/multi-variate-gan/inputs/normalization/target_spatial_norm_all_gcm_mean.nc" 
...
def compute_signal(df, quantiles, historical_period, future_period, output_varname = config["output_varname"]):
...
for gcm in gcms:
    ...
    output_hist = xr.concat([predict_parallel_resid_v280325(gan, unet,
                                   ...
                                   output_shape.sel(time = historical_period), 64, orog.values, he.values, vegt.values, model_type='GAN', output_add_factor = output_norm, 
                                                            varname = config["output_varname"], output_means =output_means, output_stds = output_stds) for i in range(1)],
                            dim ="member")
    output_hist_reg = xr.concat([predict_parallel_resid_v280325(gan, unet,
                                   stacked_X.sel( GCM =gcm).transpose('time','lat','lon','channel').sel(time = historical_period).values,
                                   output_shape.sel(time = historical_period), 64, orog.values, he.values, vegt.values, model_type='UNET', output_add_factor = output_norm,
                                                               varname = config["output_varname"], output_means =output_means, output_stds = output_stds) for i in range(1)],
                            dim ="member")

    output_future = xr.concat([predict_parallel_resid_v280325(gan, unet,
                                         ...
                                         vegt.values, model_type='GAN', output_add_factor = output_norm,
                                                             varname = config["output_varname"], output_means =output_means, output_stds = output_stds) for i in range(1)], dim ="member")
    output_future_reg = xr.concat([predict_parallel_resid_v280325(gan, unet,
                                         stacked_X.sel(GCM=gcm).transpose('time', 'lat', 'lon', 'channel').sel(
                                             time=future_period).values,
                                         output_shape.sel(time=future_period), 64, orog.values, he.values,
                                         vegt.values, model_type='UNET', output_add_factor = output_norm,
                                                                 varname = config["output_varname"], output_means =output_means, output_stds = output_stds) for i in range(1)], dim ="member")
    
    output_future2 = xr.concat([predict_parallel_resid_v280325(gan, unet,
                                         stacked_X.sel(GCM=gcm).transpose('time', 'lat', 'lon', 'channel').sel(
                                             time=future_period2).values,
                                         output_shape.sel(time=future_period2), 64, orog.values, he.values,
                                         vegt.values, model_type='GAN', output_add_factor = output_norm,
                                                              varname = config["output_varname"], output_means =output_means, output_stds = output_stds) for i in range(1)], dim ="member")
    output_future_reg2 = xr.concat([predict_parallel_resid_v280325(gan, unet,
                                         stacked_X.sel(GCM=gcm).transpose('time', 'lat', 'lon', 'channel').sel(
                                             time=future_period2).values,
                                         output_shape.sel(time=future_period2), 64, orog.values, he.values,
                                         vegt.values, model_type='UNET', output_add_factor = output_norm,
